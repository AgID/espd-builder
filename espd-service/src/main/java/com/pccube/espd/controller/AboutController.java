...
import com.pccube.espd.delegate.AboutServiceDelegate;
...
public class AboutController extends GenericController {
    ...
    @Autowired
    private AboutServiceDelegate delegate;
    

    @PostMapping(value = "/info")
    @ResponseBody
    public ResponseEntity<Response> info(HttpServletRequest request, HttpServletResponse response) {

		logger.info(LOG_START_REST_CONTROLLER + "/about/info");
    	Response serviceResponse = new Response();
		try {
			logger.info(LOG_SERVICE_CALL);
			setResponse(serviceResponse, delegate.info(), RESPONSE_OK_CODE, RESPONSE_OK_DESCRIPTION);
