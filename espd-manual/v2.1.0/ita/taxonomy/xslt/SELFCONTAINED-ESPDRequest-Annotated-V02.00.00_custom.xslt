...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="table:table-row/table:table-cell">
		...
		<xsl:choose>
			...
			<xsl:when test="$p = '{ADDITIONAL_DESCRIPTION_LINE}'">
	...
	<xsl:template name="createIndex">
		<cbc:Index>
			<xsl:value-of select="espd:getCellContent(., 1)"/>
		</cbc:Index>
	</xsl:template>
	<xsl:template name="createID">
		<cbc:ID schemeID="CriteriaTaxonomy" schemeAgencyID="EU-COM-GROW" schemeVersionID="2.1.0">
			<xsl:value-of select="espd:getCellContent(., 24)"/>
		</cbc:ID>
	</xsl:template>
	<xsl:template name="createTypeCode">
		<xsl:variable name="code" select="espd:getCellContent(., 25)"/>
			...
			<xsl:choose>
				...
				<xsl:when test="text:p = '{REQUIREMENT_GROUP'">
					...
					<cbc:TypeCode listID="CriterionElementType" listAgencyID="EU-COM-GROW"
						listVersionID="2.1.0">REQUIREMENT_GROUP</cbc:TypeCode>
				</xsl:when>
				<xsl:when test="text:p = '{REQUIREMENT_SUBGROUP'">
					<cbc:PropertyGroupTypeCode listID="PropertyGroupType" listAgencyID="EU-COM-GROW"
						listVersionID="2.1.0">
						<xsl:value-of select="$code"/>
					</cbc:PropertyGroupTypeCode>
					<cbc:TypeCode listID="CriterionElementType" listAgencyID="EU-COM-GROW"
						listVersionID="2.1.0">REQUIREMENT_SUBGROUP</cbc:TypeCode>
				</xsl:when>
				<xsl:when
					test="text:p = '{QUESTION_GROUP'">
					<cbc:PropertyGroupTypeCode listID="PropertyGroupType" listAgencyID="EU-COM-GROW"
						listVersionID="2.1.0">
						<xsl:value-of select="$code"/>
					</cbc:PropertyGroupTypeCode>
					<cbc:TypeCode listID="CriterionElementType" listAgencyID="EU-COM-GROW"
						listVersionID="2.1.0">QUESTION_GROUP</cbc:TypeCode>
				</xsl:when>
				<xsl:when
					test="text:p = '{QUESTION_SUBGROUP'">
					<cbc:PropertyGroupTypeCode listID="PropertyGroupType" listAgencyID="EU-COM-GROW"
						listVersionID="2.1.0">
						<xsl:value-of select="$code"/>
					</cbc:PropertyGroupTypeCode>
					<cbc:TypeCode listID="CriterionElementType" listAgencyID="EU-COM-GROW"
						listVersionID="2.1.0">QUESTION_SUBGROUP</cbc:TypeCode>
				</xsl:when>
	...
	<xsl:template name="createDescription">
		<cbc:Description languageID="EN">
			<xsl:value-of select="espd:getCellContent(., 20)"/>
		</cbc:Description>
		<cbc:Description languageID="IT">
			<xsl:value-of select="espd:getCellContent(., 19)"/>
		</cbc:Description>
	</xsl:template>
	<xsl:template name="createCardinality">
		<cbc:Cardinality>
	...
	<xsl:template name="createSubCriterion">
		...
		<xsl:call-template name="createDescription"/>
		<xsl:call-template name="createCardinality"/>
	</xsl:template>
	...
	<xsl:template name="createDataTypeValue">
		<xsl:variable name="captionValue" select="espd:getCellContent(., 23)"/>
		<xsl:choose>
	...
	<xsl:template name="createExpectedRequirementValue">
		<xsl:for-each select="ancestor-or-self::table:table-row/table:table-cell">
			<xsl:if test="text:p = '{REQUIREMENT}'">
				...
				<xsl:variable name="codeList" select="espd:getCellContent(., 26)"/>
				<xsl:variable name="propertyDataType" select="espd:getCellContent(., 23)"/>
				<xsl:variable name="value" select="espd:getCellContent(., 21)"/>
				<xsl:choose>
					...
					<xsl:when test="$propertyDataType = 'CODE'">
						...
						<xsl:if test="$codeList != ''">
							<cbc:ExpectedCode
								listAgencyID="EU-COM-GROW" listVersionID="2.1.0">
								<xsl:attribute name="listID"><xsl:value-of select="$codeList"/></xsl:attribute>
								<xsl:value-of select="$value"/>
							</cbc:ExpectedCode>
						</xsl:if>
					</xsl:when>
	...
	<xsl:template name="createResponseQuestionValue">
		<xsl:for-each select="ancestor-or-self::table:table-row/table:table-cell">
			<xsl:if test="text:p = '{QUESTION}'">
				<xsl:variable name="codeList" select="espd:getCellContent(., 26)"/>
				<xsl:variable name="propertyDataType" select="espd:getCellContent(., 23)"/>
				<xsl:choose>
					<xsl:when test="$propertyDataType = 'CODE' or $propertyDataType = 'ECONOMIC_OPERATOR_IDENTIFIER'">
						<xsl:if test="$codeList != ''">
							<cbc:ResponseCode
								listAgencyID="EU-COM-GROW" listVersionID="2.1.0">
								<xsl:attribute name="listID"><xsl:value-of select="$codeList"/></xsl:attribute>
	...
	<xsl:template name="createProperty">
		...
		<xsl:call-template name="createResponseQuestionValue"/>
		<xsl:call-template name="createCardinality"/>
	</xsl:template>
</xsl:stylesheet>
